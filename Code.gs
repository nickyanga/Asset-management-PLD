...
function getAssetsSheet_() {
  ...
  if (!sheet) throw new Error('Assets sheet not found. Please create it with headers.');
  return sheet;
}
...
function getTransactionsSheet_() {
  ...
  if (!sheet) throw new Error('Transactions sheet not found. Please create it with headers.');
  return sheet;
}
...
function sendLoanReceipt_(data) {
  var subject = 'PLD Loan Receipt \u2013 ' + data.assetTag + ' ' + data.deviceName;
  var body = [
    'PLD Loan Receipt',
    '================',
    '',
...
function sendReturnReceipt_(data) {
  var subject = 'PLD Return Receipt \u2013 ' + data.assetTag + ' ' + data.deviceName;
...
function getAssetByTag(assetTag) {
  var tag = String(assetTag || '').trim().toUpperCase();
  var sheet = getAssetsSheet_();
  var rows = getSheetData_(sheet);
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i][0] || '').trim().toUpperCase() === tag) {
      return {
        ...
        dateAdded:    rows[i][8] ? Utilities.formatDate(new Date(rows[i][8]), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss') : '',
        lastModified: rows[i][9] ? Utilities.formatDate(new Date(rows[i][9]), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss') : ''
      };
    }
  }
  return null;
...
function getAllAssets() {
  ...
  return rows.map(function(row) {
    return {
      ...
      notes:        String(row[7] || '').trim(),
      dateAdded:    row[8] ? Utilities.formatDate(new Date(row[8]), Session.getScriptTimeZone(), 'yyyy-MM-dd') : '',
      lastModified: row[9] ? Utilities.formatDate(new Date(row[9]), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss') : ''
    };
  }).filter(function(a) { return a.assetTag !== ''; });
}
...
function getAllTransactions() {
  ...
  var txns = rows.map(function(row) {
    return {
      ...
      loanDate:       row[7] ? Utilities.formatDate(new Date(row[7]), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss') : '',
      returnDate:     row[8] ? Utilities.formatDate(new Date(row[8]), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss') : '',
      type:           String(row[9] || '').trim(),
      adminNotes:     String(row[10] || '').trim()
    };
  }).filter(function(t) { return t.transactionId !== ''; });
...
function updateAsset(params) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (String(data[i][0] || '').trim().toUpperCase() === tag) {
      var rowNum = i + 2;
      var row = data[i];
...
function importAssetsFromCSV(csvString) {
  ...
  if (colIndex.assetTag === -1) return { imported: 0, skipped: 0, errors: ['CSV missing required "AssetTag" column.'] };
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    var tag = colIndex.assetTag >= 0 ? String(cols[colIndex.assetTag] || '').trim() : '';
    if (!tag) {
    ...
    try {
      addAsset({
        assetTag:     tag,
        serialNumber: colIndex.serialNumber >= 0 ? (cols[colIndex.serialNumber] || '') : '',
        deviceName:   colIndex.deviceName   >= 0 ? (cols[colIndex.deviceName]   || '') : '',
        model:        colIndex.model        >= 0 ? (cols[colIndex.model]        || '') : '',
        category:     colIndex.category     >= 0 ? (cols[colIndex.category]     || '') : '',
        condition:    colIndex.condition    >= 0 ? (cols[colIndex.condition]    || 'Good') : 'Good',
...
function submitLoan(loanData) {
  ...
  try {
    ...
    if (asset.status !== 'Available') throw new Error('Asset "' + tag + '" is not available (current status: ' + asset.status + ').');
    ...
    sendLoanReceipt_({
      ...
      loanDate:      Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss')
    });
...
function lookupLoanByAssetTag(assetTag) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var type       = String(row[9] || '').trim();
    var returnDate = String(row[8] || '').trim();
    var rowTag     = String(row[1] || '').trim().toUpperCase();
...
function submitReturn(returnData) {
  ...
  try {
    ...
    if (targetRow === -1) throw new Error('Open loan transaction "' + txnId + '" not found.');
